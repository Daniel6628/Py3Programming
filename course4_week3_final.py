...
class WOFHumanPlayer(WOFPlayer):
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        enter_str = input("Guess a letter, phrase, or type 'exit' or 'pass': ")
        return enter_str
...
class WOFComputerPlayer(WOFPlayer):
    ...
    def getPossibleLetters(self, guessed):
        ...
        for letter in LETTERS:
            if letter not in guessed and letter not in VOWELS:
                CanBeGuessedLetters.append(letter)
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        CanBeGuessedLetters = self.getPossibleLetters(guessed)
        if CanBeGuessedLetters == []:
            return 'pass'
        else:
            ...
            if the_value == True:
                ...
                while(0 <= i <= len(self.SORTED_FREQUENCIES)-1):
                    if self.SORTED_FREQUENCIES[i] in CanBeGuessedLetters:
                        return self.SORTED_FREQUENCIES[i]
                    else:
                        i-=1
                        continue
            else:
                rand_letter = random.choice(CanBeGuessedLetters)
                return rand_letter
