...
'''
The dictionary, medals, shows the medal count for six countries during the Rio Olympics. Sort the country names so they appear alphabetically. 
...
medal_ct = sorted(medals, key = lambda keys: medals[keys], reverse = True)
top_three = medal_ct[:3]
...
most_needed = sorted(groceries, key = lambda food: groceries[food], reverse = True)
...
Then, use this function to sort the list of ids stored in the variable, ids, from lowest to highest. Save this sorted list in the variable, 
...
sorted_id = sorted(ids, key = lambda num: str(num)[-4:-1])
...
lambda_sort = sorted(ex_lst, key = lambda astr: astr[1])
